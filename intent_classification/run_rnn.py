...
from utility import read_vocab, batch_iter, process_file, build_vocab
...
def test():
    ...
    y_pred = np.zeros(shape=(len(x_test), len(categories)), dtype=np.int32)  # 保存预测结果
    for i in range(num_batch):  # 逐批次处理
        ...
        y_pred[start_id:end_id] = session.run(tf.round(tf.nn.sigmoid(model.logits)), feed_dict=feed_dict)
